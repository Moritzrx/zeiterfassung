...
/** Screen 2: Woche. Level-Ring, Wochenbalken, Tätigkeiten gegen ihre Ziele, Restlaufzeit, Woche durchgehen. */
export function WocheScreen(): ReactElement {
  ...
  const aktuellesLevel = level(produktiv)
  const geschafft = aktuellesLevel >= ziel
  ...
  if (aktuelleWoche) {
    const zielSekunden = ziel * SEKUNDEN_PRO_LEVEL
    if (produktiv >= zielSekunden) {
      restlaufzeit = `Wochenziel geschafft. Noch ${stundenText(bisNaechstesLevel(produktiv))} h bis Level ${aktuellesLevel + 1}.`
    } else {
      const rest = zielSekunden - produktiv
      const tageUebrig = verbleibendeArbeitstage(new Date(jetzt))
      restlaufzeit =
        tageUebrig === 0
          ? `Wochenende. Noch ${stundenText(rest)} h bis Level ${ziel}.`
          : `Noch ${stundenText(rest)} h bis Level ${ziel}, bei ${tageUebrig} verbleibenden ${
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <section className="flex flex-col items-center">
        <LevelRing produktivSekunden={produktiv} zielLevel={ziel}>
          <p className="text-sm text-mute">Level</p>
          <p className="text-[64px] leading-none font-light">
            <AnimierteZahl wert={aktuellesLevel} format={(n) => String(Math.round(n))} />
          </p>
          <p className="mt-2 text-sm text-mute">
            <AnimierteZahl wert={produktiv} format={stundenText} /> h produktiv
          </p>
        </LevelRing>
        <p className={`mt-4 text-sm ${geschafft ? 'text-produktiv' : 'text-mute'}`}>
          {geschafft
            ? `Level ${ziel} erreicht, Woche geschafft`
            : `Level ${aktuellesLevel}, noch ${stundenText(bisNaechstesLevel(produktiv))} h bis Level ${aktuellesLevel + 1}`}
        </p>
        {restlaufzeit && <p className="mt-1 text-sm text-dim">{restlaufzeit}</p>}
      </section>
